...
RSpec.describe Boosted::Controllers::Tabulatable, type: :controller do
  ...
  before do
    ...
    allow(Boosted::Queries::Search).to receive(:call).with(scope, search_term: anything,
  ...
  describe "MockController#index" do
    ...
    context "when no tabulate fields are defined" do
      ...
      it "calls Boosted::Queries::Search with correct params" do
        request
        expect(Boosted::Queries::Search).to have_received(:call).with(scope, search_term: nil,
    ...
    context "when tabulate fields are defined" do
      ...
      context "when passing non mapped sort names" do
        ...
        it "calls Boosted::Queries::Sort with correct params" do
          request
          expect(Boosted::Queries::Sort).to have_received(:call).with(scope, sort_key: "email",
                                                                             sort_direction:)
        end
      end
      ...
      context "when passing search params" do
        ...
        it "calls Boosted::Queries::Search with correct params" do
          request
          expect(Boosted::Queries::Search).to have_received(:call).with(scope, search_term: "John",
                                                                               model_search_scope: :search)
        end
      end
