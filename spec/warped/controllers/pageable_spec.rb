...
RSpec.describe Warped::Controllers::Pageable, type: :controller do
  ...
  describe "MockController#index" do
    ...
    context "when the per_page parameter is present" do
      ...
      it "calls Warped::Queries::Paginate with correct params" do
        request
        expect(Warped::Queries::Paginate).to have_received(:call).with(scope, page: 1, per_page: 20)
      end
    end
    ...
    context "when the page and per_page parameters are present" do
      ...
      it "calls Warped::Queries::Paginate with correct params" do
        request
        expect(Warped::Queries::Paginate).to have_received(:call).with(scope, page: 2, per_page: 20)
      end
    end
  end
end
